...
namespace Microsoft.Azure.SignalR.Samples.Management
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            app.OnExecute(async () =>
            {
                var connectionString = "Endpoint=https://signalabfree.service.signalr.net;AccessKey=VLt6AqsLkc2UsDAeK7QsnFN9ChCQ6LErEZnSsBFyVZ0=;Version=1.0;";//connectionStringOption.Value() ?? configuration["Azure:SignalR:ConnectionString"];
        ...
        private static async Task StartAsync(MessagePublisher publisher)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        Console.WriteLine($"broadcast message {DateTime.UtcNow.ToString("HH: mm:ss")} '{args[1]}'");
                        await publisher.SendMessages(args[0], null, args[1]);
                    }
                    else if (args.Length == 4 && args[0].Equals("send"))
                    {
                        var useSignalR = await publisher.SendMessages(args[1], args[2], args[3]);
                        var channel = useSignalR ? "signalR" : "Noti";
                        Console.WriteLine($"{args[0]} message '{args[3]}' to '{args[2]}' {channel}");
                    }
